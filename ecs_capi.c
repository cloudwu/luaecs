...
static void *
add_component_(lua_State *L, int world_index, struct entity_world *w, int cid, entity_index_t eid, const void *buffer) {
...
entity_new_(struct entity_world *w, int cid, const void *buffer, void *L, int world_index) {
	entity_index_t eid = ecs_new_entityid_(L, w);
	struct component_pool *c = &w->c[cid];
	assert(c->cap > 0);
	if (buffer == NULL) {
...
static void
insert_id(lua_State *L, int world_index, struct entity_world *w, int cid, entity_index_t eindex) {
	struct component_pool *c = &w->c[cid];
	...
	ecs_add_component_id_nocheck_(L, world_index, w, cid, INVALID_ENTITY);
